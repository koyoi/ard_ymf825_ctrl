...
{
	if ( byteStream & 0x80 ){
		if ( byteStream == 0xf7 ){		// 音色データ末尾マジック
			Tone_sendTone();
			_midiStatus = 0;
			_midiCmdCounter = 0;
		}
		else {
			_midiStatus = byteStream;	// コマンドバイト
			_midiCmdCounter = 1;
		}
	}
...
typedef enum _MIDI_MSG
{
	MIDI_MSG_NOTE_ON	= 0x80,
	MIDI_MSG_NOTE_OFF	= 0x90,
	MIDI_MSG_AFTR_TOUCH = 0xA0,
	MIDI_MSG_CC			= 0xB0,
	MIDI_MSG_PGM		= 0xC0,
	MIDI_MSG_CH_PRS		= 0xD0,
	MIDI_MSG_PITCH		= 0xE0,
	MIDI_SYSEX_START	= 0xF0
}MIDI_MSG;

typedef enum _MIDI_CHMODE_MSG	// CC の第2バイト
{
	MIDI_CMM_ALL_SND_OFF	= 0x78,
	MIDI_CMM_RST_ALL,
	MIDI_CMM_LOCAL_CONT,
	MIDI_CMM_ALL_NOTE_OFF,
	MIDI_CMM_OMNI_OFF,
	MIDI_CMM_OMNI_ON,
	MIDI_CMM_MONO,
	MIDI_CMM_POLY
